...
该处理器的概述：玄铁 C910 是面向嵌入式系统和 SoC 应用领域的 64 位超高性能嵌入式多处理器核，具有出色的性能表现。 C910 采用了 RISCV 64GC 基本指令集，和平头哥性能增强指令集，主要面向对性能要求严格的高端嵌入式应用，如人工智能、自动驾驶、移动智能终端、高性能通信、信息安全等等。

C910MP 采用同构多核架构，支持 2 个 C910 核心。每个 C910 核心采用自主设计的体系结构和微体系结构，并重点针对性能进行优化，引入 3 发射 8 执行的超标量架构、多通道的数据预取等高性能技术。C910支持实时检测并关断内部空闲功能模块，进一步降低处理器动态功耗。
